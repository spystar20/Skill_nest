...
const CertificatesShowcase = () => {
  const certificates = [{ id: 1, title: 'MERN Stack Development', issueDate: '07 August 2026', image: certificate }]
      ...
      {certificates.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {certificates.map((item) => (
            <div key={item.id} className="flex w-full max-w-[350px] flex-col gap-2">
              <div className="w-full cursor-pointer overflow-hidden rounded-lg border border-black/20 hover:brightness-95">
                <img src={item.image} className="block w-full rounded-lg" alt={item.title} />
              </div>

              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 flex-col">
                  <span className="truncate text-lg font-medium capitalize text-black/90 font-heading">{item.title}</span>
                  <span className="text-base">{item.issueDate}</span>
                </div>
